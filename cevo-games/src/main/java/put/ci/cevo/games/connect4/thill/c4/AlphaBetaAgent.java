...
import put.ci.cevo.games.connect4.thill.c4.openingBook.BookSum;

/**
 * 
 * This class is an implementation of an Alpha-Beta-Agent. It is recommended to use the implemented Methods by Agent.
 * This Agent uses Transposition-Tables, Move-ordering, opening Books, symmetries and some other techniques to get fast
 * results.
 * 
 * <br>
 * Changes: <br>
 * 12.12.2011: alphaBetaStartP1 and alphaBetaStartP2 look for the most distant loss. This makes the search a little bit
 * slower, but allows a better evaluation of the td-agent
 * 
 * @author Markus Thill
 * 
 */
...
//	private final Random rand = new Random();
	...
	private static final int[] TRANSPOSSIZE = { 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216, 33554432 };
	public static final int[] TRANSPOSBYTES = { 262144 * 99 / 8, 524288 * 99 / 8, 1048576 * 99 / 8, 2097152 * 99 / 8,
		4194304 * 99 / 8, 8388608 * 99 / 8, 16777216 * 99 / 8, 33554432 / 8 * 99 };
	...
	private static final long rnd20[] = { 0L, 2704506115994628L, 886077597871704230L, 49389502572435258L,
		82333996817139652L, 263967204879563328L, 81975673952415600L };
	private static final long rnd21[] = { 0L, 73398886193708064L, 1595863713887220963L, 46261610206381440L,
		58705059883690202L, 231696507446129885L, 22606427398883328L };
	private static final long rnd22[] = { 0L, 12595346104058426L, 4097820997223100L, 101324622437045280L,
		31779374605300125L, 35633797708573350L, 22416112427922675L };
	private static final long rnd23[] = { 0L, 36363358841386824L, 4173667863902779612L, 9131869703157656L,
		14138249969764235L, 214348955873908032L, 58547228054472360L };
	private static final long rnd24[] = { 0L, 55740094356093127L, 1777939723684020L, 362858203316162568L,
		28975890403315160L, 1242349240448115806L, 59601464106441712L };
	private static final long rnd25[] = { 0L, 9110872168202946L, 10631234269963860L, 16888881020037981L,
		1159792823631456L, 36205525950397736L, 47068546447093808L };
	private static final long rnd26[] = { 0L, 375817237236357603L, 32189775283681470L, 1493718293429439L,
		20793138156733824L, 101478045365676084L, 110552240521049760L };
	private static final long rnd10[] = { 0L, 19298943901485610L, 6548220796761019L, 1777628907278452803L,
		11891769178478592L, 3564258696970080L, 236708853179436288L };
	private static final long rnd11[] = { 0L, 349182760342233125L, 429289086240375L, 121921717543355343L,
		31495429917193824L, 5694462647075520L, 30758051047680284L };
	private static final long rnd12[] = { 0L, 1365712501364505581L, 17363511325679223L, 119226791868480L,
		6220173073409360L, 11647770880598424L, 24507207907919492L };
	private static final long rnd13[] = { 0L, 551903736927872L, 2097977134396858L, 3108717381973636075L,
		25389306976498643L, 254362479754036508L, 119080142037405540L };
	private static final long rnd14[] = { 0L, 65628472867223040L, 116416206906490816L, 130703539652185785L,
		1541174198942728362L, 37852277734190556L, 22426187114508354L };
	private static final long rnd15[] = { 0L, 290694253237906321L, 3460150747465650L, 12108862858045894L,
		124792798959719156L, 6572334569141376L, 2726416766762766L };
	private static final long rnd16[] = { 0L, 378828340783306008L, 72087612995472200L, 113983283880328672L,
		376285078915283L, 62397498210717124L, 1193066389676430L };
	...
	public static AlphaBetaAgent createDefaultAgent() {
		return createAgent(false, true);
	}

	public static AlphaBetaAgent createAgent(Connect4PerfectPlayer.Randomization randomization) {
		switch (randomization) {
		case RANDOMIZE_EQUAL_MOVES:
			return createAgent(false, true);
		case RANDOMIZE_LOSSES:
			return createAgent(true, false);
		case RANDOMIZE_LOSSES_AND_MOVES:
			return createAgent(true, true);
		default:
			return createDefaultAgent();
		}
	}

	public static AlphaBetaAgent createAgent(boolean randomizeLosses, boolean randomizeEqualMoves) {
		...
		alphaBetaStd.randomizeEqualMoves(randomizeEqualMoves);
		alphaBetaStd.randomizeLosses(randomizeLosses);
		return alphaBetaStd;
	}
	...
	/**
	 * The difficulty of the AlphaBeta-Player is distinguished by the search Depth
	 * 
	 * @param searchDepth
	 */
	 ...
	 * @param player
	 * @param arr
	 *            array, because it is used as return-value. Contains one winning-Column
	 * @return number of winning columns
	 */
	private int hasWin(int player, int[] arr) {
		int x, j = 0;
		for (x = 0; x < 7; x++) {
			if (colHeight[x] < 6) {
	...
	/**
	 * This Method (the source) was generated with a C-program, because its source-code is very long. <br>
	...
	protected int findThreat(int player, int startWithCol) {
		// Pr�fen, ob eine Drohung erstellt werden kann
		...
		case 1:
			...
			case 1:
				...
				if ((temp & 0x20000100000L) == 0x20000100000L && colHeight[2] < 2) {
					return 1;
				}
				if ((temp & 0x20008000000L) == 0x20008000000L && colHeight[3] < 3) {
					return 1;
				}
				if ((temp & 0x10400000L) == 0x10400000L && (colHeight[0] < 1 || colHeight[4] < 1)) {
					return 1;
				}
				if ((temp & 0x10000400000L) == 0x10000400000L && colHeight[2] < 1) {
					return 1;
				}
				if ((temp & 0x10010000000L) == 0x10010000000L && colHeight[3] < 1) {
					return 1;
				}
			...
			case 2:
				...
				if ((temp & 0x4080000L) == 0x4080000L && (colHeight[0] < 1 || colHeight[4] < 5)) {
					return 1;
				}
				if ((temp & 0x10000080000L) == 0x10000080000L && colHeight[2] < 3) {
					return 1;
				}
				if ((temp & 0x10004000000L) == 0x10004000000L && colHeight[3] < 4) {
					return 1;
				}
				if ((temp & 0x8200000L) == 0x8200000L && (colHeight[0] < 2 || colHeight[4] < 2)) {
					return 1;
				}
			...
			case 3:
				...
				if ((temp & 0x4100000L) == 0x4100000L && (colHeight[0] < 3 || colHeight[4] < 3)) {
					return 1;
				}
				...
				break;
			case 4:
				if ((temp & 0x2080000L) == 0x2080000L && (colHeight[0] < 4 || colHeight[4] < 4)) {
					return 1;
				}
				...
				if ((temp & 0x4200000L) == 0x4200000L && (colHeight[0] < 5 || colHeight[4] < 1)) {
					return 1;
				}
				...
				break;
			case 5:
				if ((temp & 0x1040000L) == 0x1040000L && (colHeight[0] < 5 || colHeight[4] < 5)) {
					return 1;
				}
		...
		case 2:
			...
			case 1:
				...
				if ((temp & 0x400400000L) == 0x400400000L && (colHeight[0] < 1 || colHeight[4] < 1)) {
					return 2;
				}
				if ((temp & 0x10000400000L) == 0x10000400000L && colHeight[1] < 1) {
					return 2;
				}
				if ((temp & 0x10400000000L) == 0x10400000000L && colHeight[3] < 1) {
					return 2;
				}
				if ((temp & 0x410000L) == 0x410000L && (colHeight[1] < 1 || colHeight[5] < 1)) {
					return 2;
				}
			...
			case 2:
				...
				if ((temp & 0x102000L) == 0x102000L && (colHeight[1] < 1 || colHeight[5] < 5)) {
					return 2;
				}
				...
				if ((temp & 0x200200000L) == 0x200200000L && (colHeight[0] < 2 || colHeight[4] < 2)) {
					return 2;
				}
				...
				if ((temp & 0x208000L) == 0x208000L && (colHeight[1] < 2 || colHeight[5] < 2)) {
					return 2;
				}
				...
				if ((temp & 0x20000100000L) == 0x20000100000L && colHeight[1] < 1) {
					return 2;
				}
				if ((temp & 0x20400000000L) == 0x20400000000L && colHeight[3] < 3) {
					return 2;
				}
				break;
			case 3:
				if ((temp & 0x80200000L) == 0x80200000L && (colHeight[0] < 5 || colHeight[4] < 1)) {
					return 2;
				}
				...
				if ((temp & 0x200080000L) == 0x200080000L && (colHeight[0] < 1 || colHeight[4] < 5)) {
					return 2;
				}
				if ((temp & 0x100100000L) == 0x100100000L && (colHeight[0] < 3 || colHeight[4] < 3)) {
					return 2;
				}
				...
				if ((temp & 0x104000L) == 0x104000L && (colHeight[1] < 3 || colHeight[5] < 3)) {
					return 2;
				}
				...
				if ((temp & 0x10000080000L) == 0x10000080000L && colHeight[1] < 2) {
					return 2;
				}
				if ((temp & 0x10200000000L) == 0x10200000000L && colHeight[3] < 4) {
					return 2;
				}
				...
				break;
			case 4:
				if ((temp & 0x80080000L) == 0x80080000L && (colHeight[0] < 4 || colHeight[4] < 4)) {
					return 2;
				}
				...
				if ((temp & 0x82000L) == 0x82000L && (colHeight[1] < 4 || colHeight[5] < 4)) {
					return 2;
				}
				...
				if ((temp & 0x108000L) == 0x108000L && (colHeight[1] < 5 || colHeight[5] < 1)) {
					return 2;
				}
				...
				break;
			case 5:
				if ((temp & 0x40040000L) == 0x40040000L && (colHeight[0] < 5 || colHeight[4] < 5)) {
					return 2;
				}
				...
				if ((temp & 0x41000L) == 0x41000L && (colHeight[1] < 5 || colHeight[5] < 5)) {
					return 2;
				}
		...
		case 3:
			...
			case 1:
				...
				if ((temp & 0x410000000L) == 0x410000000L && (colHeight[0] < 1 || colHeight[4] < 1)) {
					return 3;
				}
				if ((temp & 0x10010000000L) == 0x10010000000L && colHeight[1] < 1) {
					return 3;
				}
				if ((temp & 0x10400000000L) == 0x10400000000L && colHeight[2] < 1) {
					return 3;
				}
				if ((temp & 0x10010000L) == 0x10010000L && (colHeight[1] < 1 || colHeight[5] < 1)) {
					return 3;
				}
				if ((temp & 0x400010000L) == 0x400010000L && colHeight[2] < 1) {
					return 3;
				}
				if ((temp & 0x10400L) == 0x10400L && (colHeight[2] < 1 || colHeight[6] < 1)) {
					return 3;
				}
				...
				break;
			case 2:
				if ((temp & 0x84000000L) == 0x84000000L && (colHeight[0] < 5 || colHeight[4] < 1)) {
					return 3;
				}
				...
				if ((temp & 0x4080L) == 0x4080L && (colHeight[2] < 1 || colHeight[6] < 5)) {
					return 3;
				}
				...
				if ((temp & 0x208000000L) == 0x208000000L && (colHeight[0] < 2 || colHeight[4] < 2)) {
					return 3;
				}
				...
				if ((temp & 0x8008000L) == 0x8008000L && (colHeight[1] < 2 || colHeight[5] < 2)) {
					return 3;
				}
				if ((temp & 0x200008000L) == 0x200008000L && colHeight[2] < 2) {
					return 3;
				}
				if ((temp & 0x8200L) == 0x8200L && (colHeight[2] < 2 || colHeight[6] < 2)) {
					return 3;
				}
				...
				break;
			case 3:
				if ((temp & 0x2008000L) == 0x2008000L && (colHeight[1] < 5 || colHeight[5] < 1)) {
					return 3;
				}
				...
				if ((temp & 0x8002000L) == 0x8002000L && (colHeight[1] < 1 || colHeight[5] < 5)) {
					return 3;
				}
				if ((temp & 0x104000000L) == 0x104000000L && (colHeight[0] < 3 || colHeight[4] < 3)) {
					return 3;
				}
				...
				if ((temp & 0x4004000L) == 0x4004000L && (colHeight[1] < 3 || colHeight[5] < 3)) {
					return 3;
				}
				if ((temp & 0x100004000L) == 0x100004000L && colHeight[2] < 3) {
					return 3;
				}
				if ((temp & 0x4100L) == 0x4100L && (colHeight[2] < 3 || colHeight[6] < 3)) {
					return 3;
				}
				...
				if ((temp & 0x20008000000L) == 0x20008000000L && colHeight[1] < 1) {
					return 3;
				}
				if ((temp & 0x20400000000L) == 0x20400000000L && colHeight[2] < 2) {
					return 3;
				}
				break;
			case 4:
				if ((temp & 0x82000000L) == 0x82000000L && (colHeight[0] < 4 || colHeight[4] < 4)) {
					return 3;
				}
				...
				if ((temp & 0x2002000L) == 0x2002000L && (colHeight[1] < 4 || colHeight[5] < 4)) {
					return 3;
				}
				if ((temp & 0x80002000L) == 0x80002000L && colHeight[2] < 4) {
					return 3;
				}
				if ((temp & 0x2080L) == 0x2080L && (colHeight[2] < 4 || colHeight[6] < 4)) {
					return 3;
				}
				...
				if ((temp & 0x4200L) == 0x4200L && (colHeight[2] < 5 || colHeight[6] < 1)) {
					return 3;
				}
				...
				if ((temp & 0x204000000L) == 0x204000000L && (colHeight[0] < 1 || colHeight[4] < 5)) {
					return 3;
				}
				...
				if ((temp & 0x10004000000L) == 0x10004000000L && colHeight[1] < 2) {
					return 3;
				}
				if ((temp & 0x10200000000L) == 0x10200000000L && colHeight[2] < 3) {
					return 3;
				}
				break;
			case 5:
				if ((temp & 0x41000000L) == 0x41000000L && (colHeight[0] < 5 || colHeight[4] < 5)) {
					return 3;
				}
				...
				if ((temp & 0x1001000L) == 0x1001000L && (colHeight[1] < 5 || colHeight[5] < 5)) {
					return 3;
				}
				if ((temp & 0x40001000L) == 0x40001000L && colHeight[2] < 5) {
					return 3;
				}
				if ((temp & 0x1040L) == 0x1040L && (colHeight[2] < 5 || colHeight[6] < 5)) {
					return 3;
				}
		...
		case 4:
			...
			case 1:
				...
				if ((temp & 0x10400000L) == 0x10400000L && (colHeight[1] < 1 || colHeight[5] < 1)) {
					return 4;
				}
				...
				if ((temp & 0x400400L) == 0x400400L && (colHeight[2] < 1 || colHeight[6] < 1)) {
					return 4;
				}
				...
				break;
			case 2:
				if ((temp & 0x2100000L) == 0x2100000L && (colHeight[1] < 5 || colHeight[5] < 1)) {
					return 4;
				}
				...
				if ((temp & 0x8200000L) == 0x8200000L && (colHeight[1] < 2 || colHeight[5] < 2)) {
					return 4;
				}
				...
				if ((temp & 0x200200L) == 0x200200L && (colHeight[2] < 2 || colHeight[6] < 2)) {
					return 4;
				}
				...
				break;
			case 3:
				if ((temp & 0x80200L) == 0x80200L && (colHeight[2] < 5 || colHeight[6] < 1)) {
					return 4;
				}
				...
				if ((temp & 0x200080L) == 0x200080L && (colHeight[2] < 1 || colHeight[6] < 5)) {
					return 4;
				}
				if ((temp & 0x4100000L) == 0x4100000L && (colHeight[1] < 3 || colHeight[5] < 3)) {
					return 4;
				}
				...
				if ((temp & 0x100100L) == 0x100100L && (colHeight[2] < 3 || colHeight[6] < 3)) {
					return 4;
				}
				...
				break;
			case 4:
				if ((temp & 0x2080000L) == 0x2080000L && (colHeight[1] < 4 || colHeight[5] < 4)) {
					return 4;
				}
				...
				if ((temp & 0x80080L) == 0x80080L && (colHeight[2] < 4 || colHeight[6] < 4)) {
					return 4;
				}
				...
				if ((temp & 0x8100000L) == 0x8100000L && (colHeight[1] < 1 || colHeight[5] < 5)) {
					return 4;
				}
				...
				break;
			case 5:
				if ((temp & 0x1040000L) == 0x1040000L && (colHeight[1] < 5 || colHeight[5] < 5)) {
					return 4;
				}
				...
				if ((temp & 0x40040L) == 0x40040L && (colHeight[2] < 5 || colHeight[6] < 5)) {
					return 4;
				}
		...
		case 5:
			...
			case 1:
				...
				if ((temp & 0x410000L) == 0x410000L && (colHeight[2] < 1 || colHeight[6] < 1)) {
					return 5;
				}
				if ((temp & 0x10010000L) == 0x10010000L && colHeight[3] < 1) {
					return 5;
				...
				break;
			case 2:
				if ((temp & 0x84000L) == 0x84000L && (colHeight[2] < 5 || colHeight[6] < 1)) {
					return 5;
				}
				...
				if ((temp & 0x208000L) == 0x208000L && (colHeight[2] < 2 || colHeight[6] < 2)) {
					return 5;
				}
				if ((temp & 0x8008000L) == 0x8008000L && colHeight[3] < 2) {
					return 5;
			...
			case 3:
				...
				if ((temp & 0x104000L) == 0x104000L && (colHeight[2] < 3 || colHeight[6] < 3)) {
					return 5;
				}
				if ((temp & 0x4004000L) == 0x4004000L && colHeight[3] < 3) {
					return 5;
				...
				break;
			case 4:
				if ((temp & 0x82000L) == 0x82000L && (colHeight[2] < 4 || colHeight[6] < 4)) {
					return 5;
				}
				if ((temp & 0x2002000L) == 0x2002000L && colHeight[3] < 4) {
					return 5;
				...
				if ((temp & 0x204000L) == 0x204000L && (colHeight[2] < 1 || colHeight[6] < 5)) {
					return 5;
				}
				if ((temp & 0x10004000L) == 0x10004000L && colHeight[3] < 2) {
					return 5;
				...
				break;
			case 5:
				if ((temp & 0x41000L) == 0x41000L && (colHeight[2] < 5 || colHeight[6] < 5)) {
					return 5;
				}
				if ((temp & 0x1001000L) == 0x1001000L && colHeight[3] < 5) {
					return 5;
	...
	/**
	 * This Method (the source) was generated with a C-program, because its source-code is very long. Can only be used
	 * for Player 1.<br>
	...
	protected int findOddThreatP1(int startWith) {
		// Diese Methode sucht nach ungeraden Drohung, JEDOCH nur f�r den
		// Anziehenden
		switch (startWith) {
		case 0:
			switch (colHeight[0]) {
			case 0:
				if ((fieldP1 & 0x410100000L) == 0x400100000L && colHeight[2] < 2) {
					return 0;
				}
				break;
			case 1:
				...
				if ((fieldP1 & 0x204100000L) == 0x204000000L && colHeight[3] < 4) {
					return 0;
				}
				break;
			case 2:
				if ((fieldP1 & 0x104040000L) == 0x100040000L && colHeight[2] < 4) {
					return 0;
				}
				if ((fieldP1 & 0x408200000L) == 0x8200000L && colHeight[1] < 2) {
					return 0;
				}
				if ((fieldP1 & 0x210200000L) == 0x200200000L && colHeight[2] < 2) {
					return 0;
				}
				if ((fieldP1 & 0x208400000L) == 0x208000000L && colHeight[3] < 2) {
					return 0;
				}
				break;
			case 3:
			...
			case 4:
				...
				if ((fieldP1 & 0x110400000L) == 0x100400000L && colHeight[2] < 2) {
					return 0;
				}
				break;
			case 5:
		...
		case 1:
			...
			case 1:
				...
				if ((fieldP1 & 0x20000100000L) == 0x20000100000L && colHeight[2] < 2) {
					return 1;
				}
				break;
			case 2:
				...
				if ((fieldP1 & 0x10004000000L) == 0x10004000000L && colHeight[3] < 4) {
					return 1;
				}
				if ((fieldP1 & 0x8200000L) == 0x8200000L && (colHeight[0] < 2 || colHeight[4] < 2)) {
					return 1;
				}
				if ((fieldP1 & 0x8000200000L) == 0x8000200000L && colHeight[2] < 2) {
					return 1;
				}
				if ((fieldP1 & 0x8008000000L) == 0x8008000000L && colHeight[3] < 2) {
					return 1;
				}
			...
			case 3:
				...
				if ((fieldP1 & 0x8000040000L) == 0x8000040000L && colHeight[2] < 4) {
					return 1;
				}
				if ((fieldP1 & 0x8400000L) == 0x8400000L && colHeight[0] < 4) {
					return 1;
				}
				if ((fieldP1 & 0x2000400000L) == 0x2000400000L && colHeight[2] < 2) {
					return 1;
				}
				...
				break;
			case 4:
				if ((fieldP1 & 0x2080000L) == 0x2080000L && (colHeight[0] < 4 || colHeight[4] < 4)) {
					return 1;
				}
				if ((fieldP1 & 0x2000080000L) == 0x2000080000L && colHeight[2] < 4) {
					return 1;
				}
				if ((fieldP1 & 0x2002000000L) == 0x2002000000L && colHeight[3] < 4) {
					return 1;
				}
				...
				if ((fieldP1 & 0x1004000000L) == 0x1004000000L && colHeight[3] < 2) {
					return 1;
				}
		...
		case 2:
			...
			case 1:
				...
				if ((fieldP1 & 0x4000800000L) == 0x4000800000L && colHeight[1] < 2) {
					return 2;
				}
				if ((fieldP1 & 0x800004000L) == 0x800004000L && colHeight[3] < 2) {
					return 2;
				}
				break;
			case 2:
				...
				if ((fieldP1 & 0x100400000L) == 0x100400000L && colHeight[0] < 4) {
					return 2;
				}
				if ((fieldP1 & 0x400100000L) == 0x400100000L && colHeight[4] < 4) {
					return 2;
				}
				if ((fieldP1 & 0x200200000L) == 0x200200000L && (colHeight[0] < 2 || colHeight[4] < 2)) {
					return 2;
				}
				if ((fieldP1 & 0x8000200000L) == 0x8000200000L && colHeight[1] < 2) {
					return 2;
				}
				if ((fieldP1 & 0x8200000000L) == 0x8200000000L && colHeight[3] < 2) {
					return 2;
				}
				if ((fieldP1 & 0x208000L) == 0x208000L && (colHeight[1] < 2 || colHeight[5] < 2)) {
					return 2;
				}
				if ((fieldP1 & 0x200008000L) == 0x200008000L && colHeight[3] < 2) {
					return 2;
				}
				...
				break;
			case 3:
				if ((fieldP1 & 0x1000200000L) == 0x1000200000L && colHeight[1] < 4) {
					return 2;
				}
				if ((fieldP1 & 0x1080000000L) == 0x1080000000L && colHeight[3] < 2) {
					return 2;
				}
				if ((fieldP1 & 0x81000L) == 0x81000L && colHeight[1] < 2) {
					return 2;
				}
				if ((fieldP1 & 0x200001000L) == 0x200001000L && colHeight[3] < 4) {
					return 2;
				}
				...
				if ((fieldP1 & 0x10000080000L) == 0x10000080000L && colHeight[1] < 2) {
					return 2;
				}
				if ((fieldP1 & 0x10200000000L) == 0x10200000000L && colHeight[3] < 4) {
					return 2;
				}
				...
				break;
			case 4:
				if ((fieldP1 & 0x80080000L) == 0x80080000L && (colHeight[0] < 4 || colHeight[4] < 4)) {
					return 2;
				}
				if ((fieldP1 & 0x2000080000L) == 0x2000080000L && colHeight[1] < 4) {
					return 2;
				}
				if ((fieldP1 & 0x2080000000L) == 0x2080000000L && colHeight[3] < 4) {
					return 2;
				}
				if ((fieldP1 & 0x82000L) == 0x82000L && (colHeight[1] < 4 || colHeight[5] < 4)) {
					return 2;
				}
				...
				if ((fieldP1 & 0x100040000L) == 0x100040000L && colHeight[0] < 2) {
					return 2;
				}
		...
		case 3:
			switch (colHeight[3]) {
			case 0:
				if ((fieldP1 & 0x4010000000L) == 0x4010000000L && colHeight[1] < 2) {
					return 3;
				}
				...
				break;
			case 1:
				if ((fieldP1 & 0x108000000L) == 0x108000000L && colHeight[0] < 4) {
					return 3;
				}
				if ((fieldP1 & 0x2100000000L) == 0x2100000000L && colHeight[2] < 2) {
					return 3;
				}
				...
				if ((fieldP1 & 0x100020000L) == 0x100020000L && colHeight[2] < 2) {
					return 3;
				}
				...
				break;
			case 2:
				if ((fieldP1 & 0x1004000000L) == 0x1004000000L && colHeight[1] < 4) {
					return 3;
				}
				...
				if ((fieldP1 & 0x208000000L) == 0x208000000L && (colHeight[0] < 2 || colHeight[4] < 2)) {
					return 3;
				}
				if ((fieldP1 & 0x8008000000L) == 0x8008000000L && colHeight[1] < 2) {
					return 3;
				}
				if ((fieldP1 & 0x8200000000L) == 0x8200000000L && colHeight[2] < 2) {
					return 3;
				}
				if ((fieldP1 & 0x8008000L) == 0x8008000L && (colHeight[1] < 2 || colHeight[5] < 2)) {
					return 3;
				}
				if ((fieldP1 & 0x200008000L) == 0x200008000L && colHeight[2] < 2) {
					return 3;
				}
				if ((fieldP1 & 0x8200L) == 0x8200L && (colHeight[2] < 2 || colHeight[6] < 2)) {
					return 3;
				}
			...
			case 3:
				...
				if ((fieldP1 & 0x400002000L) == 0x400002000L && colHeight[2] < 2) {
					return 3;
				}
				if ((fieldP1 & 0x408000000L) == 0x408000000L && colHeight[4] < 4) {
					return 3;
				}
				if ((fieldP1 & 0x420L) == 0x420L && colHeight[4] < 2) {
					return 3;
				}
				if ((fieldP1 & 0x20400000000L) == 0x20400000000L && colHeight[2] < 2) {
					return 3;
				}
				break;
			case 4:
				if ((fieldP1 & 0x82000000L) == 0x82000000L && (colHeight[0] < 4 || colHeight[4] < 4)) {
					return 3;
				}
				if ((fieldP1 & 0x2002000000L) == 0x2002000000L && colHeight[1] < 4) {
					return 3;
				}
				if ((fieldP1 & 0x2080000000L) == 0x2080000000L && colHeight[2] < 4) {
					return 3;
				}
				if ((fieldP1 & 0x2002000L) == 0x2002000L && (colHeight[1] < 4 || colHeight[5] < 4)) {
					return 3;
				}
				if ((fieldP1 & 0x80002000L) == 0x80002000L && colHeight[2] < 4) {
					return 3;
				}
				if ((fieldP1 & 0x2080L) == 0x2080L && (colHeight[2] < 4 || colHeight[6] < 4)) {
					return 3;
				}
				...
				if ((fieldP1 & 0x10004000000L) == 0x10004000000L && colHeight[1] < 2) {
					return 3;
				}
				break;
			case 5:
				...
				if ((fieldP1 & 0x102000000L) == 0x102000000L && colHeight[0] < 2) {
					return 3;
				}
				if ((fieldP1 & 0x8100000000L) == 0x8100000000L && colHeight[2] < 4) {
					return 3;
				}
				break;
			default:
		...
		case 4:
			switch (colHeight[4]) {
			case 0:
				if ((fieldP1 & 0x100400000L) == 0x100400000L && colHeight[2] < 2) {
					return 4;
				}
				break;
			case 1:
			...
			case 2:
				...
				if ((fieldP1 & 0x8200000L) == 0x8200000L && (colHeight[1] < 2 || colHeight[5] < 2)) {
					return 4;
				}
				if ((fieldP1 & 0x200200000L) == 0x200200000L && colHeight[2] < 2) {
					return 4;
				}
				if ((fieldP1 & 0x208000000L) == 0x208000000L && colHeight[3] < 2) {
					return 4;
				}
				if ((fieldP1 & 0x200200L) == 0x200200L && (colHeight[2] < 2 || colHeight[6] < 2)) {
					return 4;
				}
			...
			case 3:
				...
				if ((fieldP1 & 0x810000000L) == 0x810000000L && colHeight[3] < 2) {
					return 4;
				}
				break;
			case 4:
				if ((fieldP1 & 0x2080000L) == 0x2080000L && (colHeight[1] < 4 || colHeight[5] < 4)) {
					return 4;
				}
				...
				if ((fieldP1 & 0x80080L) == 0x80080L && (colHeight[2] < 4 || colHeight[6] < 4)) {
					return 4;
				}
				...
				if ((fieldP1 & 0x400100000L) == 0x400100000L && colHeight[2] < 2) {
					return 4;
				}
				break;
			case 5:
				...
				if ((fieldP1 & 0x204000000L) == 0x204000000L && colHeight[3] < 4) {
					return 4;
				}
				break;
			default:
		...
		case 5:
			...
			case 2:
				...
				if ((fieldP1 & 0x208000L) == 0x208000L && (colHeight[2] < 2 || colHeight[6] < 2)) {
					return 5;
				}
				if ((fieldP1 & 0x8008000L) == 0x8008000L && colHeight[3] < 2) {
					return 5;
			...
			case 3:
				...
				break;
			case 4:
				if ((fieldP1 & 0x82000L) == 0x82000L && (colHeight[2] < 4 || colHeight[6] < 4)) {
					return 5;
				}
				if ((fieldP1 & 0x2002000L) == 0x2002000L && colHeight[3] < 4) {
					return 5;
	 ...
	 * @return best move for the current player, or game-theoretic-value for the current board
	 */
	protected int rootNode(boolean retValue) {
		...
		for (x = 0; moves[x] > -1; x++) {
			...
			if (player == PLAYER1) {
				value = alphaBetaStartP2(1, alpha, beta, toZobrist(fieldP1, fieldP2), symPossible());
			} else {
				value = alphaBetaStartP1(1, alpha, beta, toZobrist(fieldP1, fieldP2), symPossible());
			}
		...
		if (!seekFarLoose && !retValue && ((player == PLAYER1 && alpha < -500) || (player == PLAYER2 && beta > 500))
	 ...
	 * @return
	 */
	private int alphaBetaStartP1(int depth, int alpha, int beta, long zobr, boolean symPos) {
		...
		if (useBook && depth < 10 && countPieces() == 8 && books != null) {
			...
			int codedPosMirrored = fieldToHuffman(getMirroredField(PLAYER2), getMirroredField(PLAYER1), true);
			int value = books.getOpeningBook().getValue(codedPos, codedPosMirrored);
			if (value == 0) {
				return 0;
			} else if (value == 1) {
				return -992;
		...
		if (useDeepBookDist && depth < 14 && countPieces() == 12 && books != null) {
			// The params must be swapped, because the database uses a other
			// representation
			int codedPos = fieldToHuffman(fieldP2, fieldP1, false);
			int codedPosMirrored = fieldToHuffman(getMirroredField(PLAYER2), getMirroredField(PLAYER1), true);
			return books.getOpeningBookDeepDist().getValue(codedPos, codedPosMirrored);
		}
		...
		if (useDeepBook && depth < 14 && countPieces() == 12 && books != null) {
			...
			int codedPos = fieldToHuffman(fieldP2, fieldP1, false);
			int codedPosMirrored = fieldToHuffman(getMirroredField(PLAYER2), getMirroredField(PLAYER1), true);
			int value = books.getOpeningBookDeep().getValue(codedPos, codedPosMirrored);
			if (value == 0) {
				return 0;
			} else if (value == 1) {
				return -988;
		...
		if (depth < 14 && !hasWin(PLAYER2)) {
			for (x = 0; moves[x] > -1; x++) {
				...
				if (anz > 1) {
					...
					if (colHeight[drlt[0]] < 5 && canWin(PLAYER1, drlt[0], colHeight[drlt[0]] + 1)) {
						fieldP1 = t;
						colHeight[moves[x]]--;
			...
			for (x = 0; moves[x] != -1; x++) {
				if (colHeight[moves[x]] < 4) {
					...
					if (canWin(PLAYER1, moves[x], colHeight[moves[x]])) {
						if (canWin(PLAYER1, moves[x], colHeight[moves[x]] + 1) && !hasWin(PLAYER2)) { // direct win ->
							colHeight[moves[x]]--;
							return 1020 - distance;
						}
					}
		...
		if (y > -1) {
			...
			value = (short) alphaBetaStartP2(depth + 1, alpha, beta, zobr ^ rnd[0][y * 6 - 1 + colHeight[y]], symPos);
			...
			if (z > -1) {
				...
				value = (short) alphaBetaStartP2(depth + 1, alpha, beta, zobr ^ rnd[0][z * 6 - 1 + colHeight[z]],
					symPos);
		...
		if (q != -1) {
			if (colHeight[q] < 4 && colHeight[q] % 2 != 0 && later == -1 && canWin(PLAYER1, q, colHeight[q] + 1)) {
				later = q;
			} else {
				t = fieldP1;
				fieldP1 |= fieldMask[q][colHeight[q]++];
				value = (short) alphaBetaStartP2(depth + 1, alpha, beta, zobr ^ rnd[0][q * 6 - 1 + colHeight[q]],
					symPos);
		...
		for (x = 0; moves[x] > -1; x++) {
			...
			// worthless
			if (colHeight[moves[x]] < 4 && colHeight[moves[x]] % 2 != 0 && later == -1
				...
				value = (short) alphaBetaStartP2(depth + 1, alpha, beta, zobr
					^ rnd[0][moves[x] * 6 - 1 + colHeight[moves[x]]], symPos);
		...
		if (later > -1) {
			...
			value = (short) alphaBetaStartP2(depth + 1, alpha, beta, zobr ^ rnd[0][later * 6 - 1 + colHeight[later]],
				symPos);
	 ...
	 * @return Value
	 */
	private int alphaBetaStartP2(int depth, int alpha, int beta, long zobr, boolean symPos) {
		int distance = depth;// (seekFarLoose ? countPieces() / looseIntervall :
							 // 0);
		if (hasWin(PLAYER2)) {
			return -1020 + distance;
		...
		if (depth < 12 && !hasWin(PLAYER1)) {
			for (x = 0; moves[x] > -1; x++) {
				...
				if (anz > 1) {
					...
					if (colHeight[drlt[0]] < 5 && canWin(PLAYER2, drlt[0], colHeight[drlt[0]] + 1)) {
						fieldP2 = t;
						colHeight[moves[x]]--;
						return -1020 + distance;
					}
			...
			// Nach Doppeldrohungen suchen, die direkt �bereinander sind
			for (x = 0; moves[x] != -1; x++) {
				if (colHeight[moves[x]] < 4) {
					colHeight[moves[x]]++;
					if (canWin(PLAYER2, moves[x], colHeight[moves[x]])) {
						if (canWin(PLAYER2, moves[x], colHeight[moves[x]] + 1) && !hasWin(PLAYER1)) {
							colHeight[moves[x]]--;
							return -1020 + distance;
						}
					}
		...
		if (y > -1) {
			...
			if (depth > 13) {
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd[1][y * 6 - 1 + colHeight[y]]);
			} else {
				value = alphaBetaStartP1(depth + 1, alpha, beta, zobr ^ rnd[1][y * 6 - 1 + colHeight[y]], symPos);
			}
			...
			if (z > -1) {
				...
				if (depth > 13) {
					value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd[1][z * 6 - 1 + colHeight[z]]);
				} else {
					value = alphaBetaStartP1(depth + 1, alpha, beta, zobr ^ rnd[1][z * 6 - 1 + colHeight[z]], symPos);
				}
		...
		if (q != -1) {
			if (colHeight[q] < 5 && colHeight[q] % 2 == 0 && later == -1 && canWin(PLAYER2, q, colHeight[q] + 1)) {
				later = q;
			} else {
				t = fieldP2;
				fieldP2 |= fieldMask[q][colHeight[q]++];
				value = alphaBetaStartP1(depth + 1, alpha, beta, zobr ^ rnd[1][q * 6 - 1 + colHeight[q]], symPos);
				fieldP2 = t;
				colHeight[q]--;
				if (value <= alpha) {
					return value;
		...
		for (x = 0; moves[x] > -1; x++) {
			...
			if (colHeight[moves[x]] < 5 && colHeight[moves[x]] % 2 == 0 && later == -1
				...
				fieldP2 |= fieldMask[moves[x]][colHeight[moves[x]]++];
				if (depth > 13) {
					value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd[1][moves[x] * 6 - 1 + colHeight[moves[x]]]);
				} else {
					value = alphaBetaStartP1(depth + 1, alpha, beta, zobr
						^ rnd[1][moves[x] * 6 - 1 + colHeight[moves[x]]], symPos);
				}
		...
		if (later > -1) {
			...
			if (depth > 13) {
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd[1][later * 6 - 1 + colHeight[later]]);
			} else {
				value = alphaBetaStartP1(depth + 1, alpha, beta, zobr ^ rnd[1][later * 6 - 1 + colHeight[later]],
					symPos);
			}
	...
	private int alphaBetaP1(int depth, int alpha, int beta, long zobr) {
		...
		if (depth == movesTillFull - 2) {
			...
			if ((f & (f - 1L)) != 0L) {
				...
				if (canWin(PLAYER2, v[0], colHeight[v[0]]) && canWin(PLAYER2, v[1], colHeight[v[1]])) {
					return -1000;
				}
				return 0;
		...
		if (depth < 24) {
			...
			if (x > -1) {
				...
				value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd[0][x * 6 - 1 + colHeight[x]]);
		...
		if (x != 3 && colHeight[3] < 6) {
			if (colHeight[3] < 4 && colHeight[3] % 2 != 0 && canWin(PLAYER1, 3, colHeight[3] + 1)) {
				later = 3;
			} else {
				t = fieldP1;
				fieldP1 |= fieldMask3[colHeight[3]++];
				value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd13[colHeight[3]]);
		...
		if (x != 4 && colHeight[4] < 6) {
			if (colHeight[4] < 4 && later == -1 && colHeight[4] % 2 != 0 && canWin(PLAYER1, 4, colHeight[4] + 1)) {
				later = 4;
			} else {
				t = fieldP1;
				fieldP1 |= fieldMask4[colHeight[4]++];
				value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd14[colHeight[4]]);
		...
		if (x != 2 && colHeight[2] < 6) {
			if (later == -1 && colHeight[2] < 4 && colHeight[2] % 2 != 0 && canWin(PLAYER1, 2, colHeight[2] + 1)) {
				later = 2;
			} else {
				t = fieldP1;
				fieldP1 |= fieldMask2[colHeight[2]++];
				value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd12[colHeight[2]]);
		...
		if (x != 5 && colHeight[5] < 6) {
			if (later == -1 && colHeight[5] < 4 && colHeight[5] % 2 != 0 && canWin(PLAYER1, 5, colHeight[5] + 1)) {
				later = 5;
			} else {
				t = fieldP1;
				fieldP1 |= fieldMask5[colHeight[5]++];
				value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd15[colHeight[5]]);
		...
		if (x != 1 && colHeight[1] < 6) {
			if (later == -1 && colHeight[1] < 4 && colHeight[1] % 2 != 0 && canWin(PLAYER1, 1, colHeight[1] + 1)) {
				later = 1;
			} else {
				t = fieldP1;
				fieldP1 |= fieldMask1[colHeight[1]++];
				value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd11[colHeight[1]]);
		...
		if (x != 6 && colHeight[6] < 6) {
			if (later == -1 && colHeight[6] < 4 && colHeight[6] % 2 != 0 && canWin(PLAYER1, 6, colHeight[6] + 1)) {
				later = 6;
			} else {
				t = fieldP1;
				fieldP1 |= fieldMask6[colHeight[6]++];
				value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd16[colHeight[6]]);
		...
		if (x != 0 && colHeight[0] < 6) {
			...
			value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd10[colHeight[0]]);
		...
		if (later != (-1)) {
			t = fieldP1;
			fieldP1 |= fieldMask[later][colHeight[later]++];
			value = alphaBetaP2(depth + 1, alpha, beta, zobr ^ rnd[0][later * 6 - 1 + colHeight[later]]);
	...
	private int alphaBetaP2(int depth, int alpha, int beta, long zobr) {
		...
		if (depth < 20) {
			...
			if (x > -1) {
				...
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd[1][x * 6 - 1 + colHeight[x]]);
		...
		if (x != 3 && colHeight[3] < 6) {
			if (colHeight[3] < 5 && canWin(PLAYER2, 3, colHeight[3] + 1)) {
				...
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd23[colHeight[3]]);
		...
		if (x != 4 && colHeight[4] < 6) {
			if (later == -1 && colHeight[4] < 5 && canWin(PLAYER2, 4, colHeight[4] + 1)) {
				later = 4;
			} else {
				t = fieldP2;
				fieldP2 |= fieldMask4[colHeight[4]++];
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd24[colHeight[4]]);
		...
		if (x != 2 && colHeight[2] < 6) {
			if (later == -1 && colHeight[2] < 5 && canWin(PLAYER2, 2, colHeight[2] + 1)) {
				later = 2;
			} else {
				t = fieldP2;
				fieldP2 |= fieldMask2[colHeight[2]++];
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd22[colHeight[2]]);
		...
		if (x != 5 && colHeight[5] < 6) {
			if (later == -1 && colHeight[5] < 5 && canWin(PLAYER2, 5, colHeight[5] + 1)) {
				later = 5;
			} else {
				t = fieldP2;
				fieldP2 |= fieldMask5[colHeight[5]++];
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd25[colHeight[5]]);
		...
		if (x != 1 && colHeight[1] < 6) {
			if (later == -1 && colHeight[1] < 5 && canWin(PLAYER2, 1, colHeight[1] + 1)) {
				later = 1;
			} else {
				t = fieldP2;
				fieldP2 |= fieldMask1[colHeight[1]++];
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd21[colHeight[1]]);
		...
		if (x != 6 && colHeight[6] < 6) {
			if (later == -1 && colHeight[6] < 5 && canWin(PLAYER2, 6, colHeight[6] + 1)) {
				later = 6;
			} else {
				t = fieldP2;
				fieldP2 |= fieldMask6[colHeight[6]++];
				value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd26[colHeight[6]]);
		...
		if (x != 0 && colHeight[0] < 6) {
			...
			value = alphaBetaP1(depth + 1, alpha, beta, zobr ^ rnd20[colHeight[0]]);
	...
	/**
	 * This method currently only works for boards with not more than 2 threats (>70% of all boards)
	 * 
	...
	public int evaluate(int curPlayer, long zobr) {
		...
		// Au�erdem Spalten markieren, in denen Drohungen
		// vorhanden sind!
		...
		// Pr�fen, wie viele Drohungen vorhanden sind
	...
	/**
	 * Find the best move for the current Player. If there are equal move, select randomly. If this board is a loss,
	 * make a completly random move. This method always rests the variable randomizeLosses.
	 * 
	 * @param table
	 * @return best move (random choice)
	 */
	private int getBestMoveRandLoss(int[][] table, RandomDataGenerator random) {
		...
		for (int i = 0; i < COLCOUNT; i++) {
			if (vals[i] != Double.NaN) {
				...
				if (curVal > bestVal) {
					...
					countEqual++;
					if (random.getRandomGenerator().nextDouble() < 1.0 / countEqual) {
						bestMove = i;
					}
				}
			}
		...
		if (bestVal <= -1000 /* && countPieces() <= 10 */) {
			...
			int x = getBestMove(table, random);
...
//	}
	...
	/**
	 * Find the best move for the current Player. If there are equal move, select randomly
	 * 
	 * @param table
	 * @return best move (random choice)
	 */
	private int getBestMoveRand(int[][] table, RandomDataGenerator random) {
		double[] vals = getNextVTable(table, false);
		int sign = (countPieces() % 2 == 0 ? 1 : -1);
		double bestVal = Double.NEGATIVE_INFINITY;
		int bestMove = -1;
	 ...
	 */
	public void setBooks(boolean useNormalBook, boolean useDeepBook, boolean useDeepBookDist) {
	...
	/**
	 * Find the best move for the current Player in method getBestMove(). If there are equal moves, select randomly. If
	 * a board is a loss, make a almost completly random move.
	 */
	...
	public int getBestMove(int[][] table, RandomDataGenerator random) {
		if (randomizeLosses) {
			return getBestMoveRandLoss(table, random);
		}
		if (randomizeEqualMoves) {
			return getBestMoveRand(table, random);
		}
		semOpDown();
		setBoard(table);
	...
	public double getScore(int[][] table, boolean useSigmoid) {
		...
		// Deep Book with Distance
			...
			int codedPos = fieldToHuffman(fieldP2, fieldP1, false);
			int codedPosMirrored = fieldToHuffman(getMirroredField(PLAYER2), getMirroredField(PLAYER1), true);
			score = books.getOpeningBookDeepDist().getValue(codedPos, codedPosMirrored);
		}
