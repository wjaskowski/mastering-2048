...
public class BoardPanel extends JPanel {
	 ...
	 */
	public BoardPanel(TetrisGUI tetris) {
		this.tetris = tetris;
	...
	public void addPiece(TileType type, int x, int y, int rotation) {
		...
		IntArrayList changedPositions = board.placeTetromino(type.getTetromino(), x, rotation);
		if (!board.isHeightExceeded()) {
			board.eraseCompleteLines(changedPositions).size();
		}
	}
	...
	public void paintComponent(Graphics g) {
		...
		if(tetris.isPaused()) {
			...
			msg = "Press Enter to Start" + (tetris.isNewGame() ? "" : " Again");
