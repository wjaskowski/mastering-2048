...
public class ResultCompMulti implements Result {
	...
	public String getResult(String title) {
		...
		str += ("Results: Actual and Expected (for Player " + singleResults[0].agents[0] + ")\n");
		str += ("-------------------------------------------\n");
		str += ("\t\tWin\tLoss\tDraw\n");
		str += ("Actual:\t\t" + numWonX + "\t" + numWonO + "\t" + numDraw + "\n");
		str += ("Expected:\t" + numExpectedWonX + "\t" + numExpectedWonO + "\t" + numExpectedDraw + "\n");
	...
	public String getSingleResult(int index, boolean allBoards) {
		return singleResults[index].getResult("Single Result: " + index, allBoards);
	}
