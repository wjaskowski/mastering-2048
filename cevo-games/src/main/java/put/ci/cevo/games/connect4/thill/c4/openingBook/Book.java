package put.ci.cevo.games.connect4.thill.c4.openingBook;

import java.io.IOException;
import java.io.InputStream;

/**
 * @author Markus Thill There are 3 different opning-books available: 1. Opening book with all positions with 8 pieces
 *         (win/draw/loss) <br>
 *         2. Opening book with all positions with 12 pieces (win/draw/loss) <br>
 *         3. Opening book with all positions with 12 pieces (win/draw/loss with exact distance) <br>
 * 
...
public class Book {
	...
	// Constants
	private static final String BOOKPATH[] = { "book.dat", "bookDeep.dat", "bookDeepDist.dat" };
	...
	public void readBook() throws IOException {
		...
		for (int i = 0; i < BOOKSIZE[bookNr]; i++) {
			...
			temp = 0;
			for (int j = 0; shiftNum >= 0; j++, shiftNum -= 8) {
				int b = file.read();
				temp |= (b << shiftNum);
			}
			book[i] = temp;
	...
	/**
	 * Search in the opening book for the coded board and return the value for this board. A fast binary-search is used.
	 * 
	 * @param codedPos
	 *            Position coded in an Integer (see class ConnectFour for details)
	 * @param codedPosMirrored
	 *            Mirrored Position coded in an Integer (see class ConnectFour for details)
