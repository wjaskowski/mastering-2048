...
/**
 * Allows different types of competitions between two Connect-Four-Agents implementing the interface Agent.
 * 
 * @author Markus Thill
 */
public class Competition implements Progress {
	 ...
	 *            Second Agent
	 * @param referee
	 *            A perfect playing AB-Agent. If this parameter is null, then the referee is initialized in the
	 *            constructor
	 */
	...
	/**
	 * Set the initial board for the next competition. If not changed, this board will be used for all competitions.
	 * 
	 * @param board
	 */
	 ...
	 * @param pieceNum
	 *            Stop game after pieceNum pieces are on the board an get the result by the referee.
	 * @param boards
	 *            boards for every single competition. If null, an empty board will be taken as initial position for
	 *            every competition.
	 * @param swapPlayers
	 *            swap players after each competition. Each agent will start the game for one board once. The number of
	 *            competitions will double in this case
	 * @param getValues
	 *            choose, if the values of the agents and the referee shall be logged for each move made. Makes a
	 *            competition take longer.
	 * @return an Object containing an Overview and every single game.
	 */
	private ResultCompMulti multiCompete(int compNum, int pieceNum, int boards[][][], boolean swapPlayers,
			boolean getValues, RandomDataGenerator random) {
		...
		for (int i = 0; i < compNum; i++) {
			...
			compResults[i] = compete(pieceNum, getValues, false, random);
	 ...
	 */
	public ResultCompMulti multiCompete(int compNum, boolean swapPlayers, boolean getValues, RandomDataGenerator random) {
		return multiCompete(compNum, 100, null, swapPlayers, getValues, random);
	}
	 ...
	 * @return an Object containing an Overview and every single game.
	 */
	public ResultCompMulti multiCompete(int compNum, int pieceNum, boolean swapPlayers, boolean getValues, RandomDataGenerator random) {
		return multiCompete(compNum, pieceNum, null, swapPlayers, getValues, random);
	}
	 ...
	 * @return an Object containing an Overview and every single game.
	 */
	public ResultCompMulti multiCompete(int boards[][][], boolean swapPlayers, boolean getValues, RandomDataGenerator random) {
		return multiCompete(boards.length, 100, boards, swapPlayers, getValues, random);
	}
	 ...
	 * @return an Object containing an Overview and every single game.
	 */
	public ResultCompMulti multiCompete(int pieceNum, int boards[][][], boolean swapPlayers, boolean getValues, RandomDataGenerator random) {
		return multiCompete(boards.length, pieceNum, boards, swapPlayers, getValues, random);
	}
	 ...
	 */
	public ResultCompSingle compete(boolean getValues, boolean stopByABWinEvaluation, RandomDataGenerator random) {
		return compete(100, getValues, stopByABWinEvaluation, random);
	}
	 ...
	 * @return an Object containing the results of this single match.
	 */
	public ResultCompSingle compete(int pieceNum, boolean getValues, boolean stopByABWinEvaluation, RandomDataGenerator random) {
		...
		for (i = 0; !gameOver; i++) {
			...
			// Get best Move
			int x = agent[curPlayer].getBestMove(board, random);
			moves[i] = x;

			boolean isABAgent = agent[curPlayer].getClass().equals(AlphaBetaAgent.class);
			if (stopByABWinEvaluation && isABAgent) {
