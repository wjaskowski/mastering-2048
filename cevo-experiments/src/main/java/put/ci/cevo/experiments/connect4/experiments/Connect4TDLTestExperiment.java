...
import put.ci.cevo.experiments.connect4.mappers.NTuplesConnect4AgentMapper;
...
import put.ci.cevo.games.board.BoardInversionQFunction;
import put.ci.cevo.games.board.BoardInversionStateValueFunction;
...
public class Connect4TDLTestExperiment implements Experiment{
	...
	private static final NTuplesConnect4AgentMapper agentMapper = new NTuplesConnect4AgentMapper(
			BoardInversionQFunction::new);
	private static final NTuplesConnect4PlayerMapper playerMapper = new NTuplesConnect4PlayerMapper();

	@Override
	public void run(String[] args) {
		ThreadedContext context = new ThreadedContext(1, 1);
		IndividualFactory<NTuples> ntuplesFactory = new Connect4NTuplesSystematicFactory(0, 0, "1111|1111|1111");
		...
		NTuples nTuples = ntuplesFactory.createRandomIndividual(context.getRandomForThread());

		System.out.println(nTuples.getMain().size());
		System.out.println(nTuples.getAll().size());
		System.out.println(nTuples);
		System.out.println(nTuples.totalWeights());

		LearnableStateValueFunction<Connect4State> afterStateValueFunction = new BoardInversionStateValueFunction<>(
				new NTuplesStateValueFunction<>(nTuples));
		//StateValueFunction<Connect4State> afterStateValueFunction =  new NTuplesStateValueFunction<>(nTuples);
		Agent<Connect4State, Connect4Action> deltaAgent = new Connect4DeltaNTuplesAgent(nTuples,
				BoardInversionQFunction::new);
	...
	private static void evalOnRand(NTuples individual, int episode, ThreadedContext context) {
		{
	...
	private static void evalOnHand(NTuples individual, int episode, ThreadedContext context) {
		Connect4Player player = playerMapper.getPhenotype(individual, context.getRandomForThread());
	...
	private static void evalOnObj(NTuples individual, int episode, ThreadedContext context) {
		//TODO
		Connect4Player player = playerMapper.getPhenotype(individual, context.getRandomForThread());
	...
	public static void main(String[] args) {
		new Connect4TDLTestExperiment().run(args);
	}
}
