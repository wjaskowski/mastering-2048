...
public class MySoftMaxAfterstatePolicy<S extends State, A extends Action> implements VFunctionControlPolicy<S, A> {
	...
	public MySoftMaxAfterstatePolicy(Environment<S, A> model, double temperature) {
		this(model, temperature, AVOID_TERMINAL_STATES_FALSE);
	}

	public MySoftMaxAfterstatePolicy(Environment<S, A> model, double temperature, boolean avoidTerminalStates) {
		this.model = model;
	...
	static double[] mysoftmax(double[] values, double temperature) {
		...
		for (int i = 0; i < values.length; ++i) {
			probabilities[i] = Math.exp((values[i] - max) / max * temperature);
